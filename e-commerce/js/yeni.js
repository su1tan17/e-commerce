...
let yeni =localStorage.getItem("products")
? JSON.parse(localStorage.getItem("products"))
: [];

function addToCart(){
  const cartItems = document.querySelector(".header-cart-count");
  const buttons=[...document.getElementsByClassName("add_to_cart")];
...
function yeniFunc(){
  ...
  yeni.forEach((item) => {
    ...
    <li class="product-item glide__slide">
      ...
      <div class="product-info">
        ...
        <div class="product-links">
          <button class="add_to_cart" data-id=${item.id}>
    ...
    `;
    yeniContainer ? (yeniContainer.innerHTML = results) : "";
    addToCart();
  });
  product2();
  productRoute();
}
export default yeniFunc;